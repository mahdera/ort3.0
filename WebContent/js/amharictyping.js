// Copyright: (c) 2006 by AmharicDictionary.com
// Product title: Amharic Phonetic Typing Javascript
// Product version details: 1.1.0, 5-01-2005 (mm-dd-yyyy)
// Product URL: http://www.amharicdictionary.com/js/
// Contact info: matt@amharicdictionary.com
// Notes: This script is free. Feel free to copy, use and change this script as 
// long as this head part remains unchanged.  If you improve on this script, please send us your updates.
...
function handleAmharicNetscape(pressEvent, amharic){
	var keyCode = 0
	...
	/*alert("type: " +pressEvent.type);
	alert("which :" +pressEvent.which);
	alert("b4 fail");*/
	
	if(window.event && !pressEvent.which) {
	...
	else if(pressEvent.type) //(pressEvent.which) 
	{
		// netscape
		keyCode = pressEvent.which; 
		isNetscape = true;
		//alert("pressEvent.which: "+pressEvent.which);
		//alert("pressEvent.type: "+pressEvent.type);
	}
	...
	if ( keyCode > 4000 || (keyCode >= 33 && keyCode <= 57 && keyCode != 44) || (keyCode >= 91 && keyCode <= 93) || (keyCode >= 60 && keyCode <= 64) || (keyCode >= 123 && keyCode <= 125))
	{
		return true;
	} 
	...
	if (keyCode != 16 && keyCode != 8 && keyCode != 13 && keyCode != 0 && keyCode != 32)//&& keyCode != 116) //shift)
	{
		...
		{  //?
			...
			}else if(amharic.value.charCodeAt(startPos-1) == 32 )
			{
				amharic.value = amharic.value + vowelCharacterChar ;
						newPos = newPos + 1;	
